...
function StateDrive(life, score,bosslife,bosslevel) {
	...
	const SURVIVE_INTERVAL = 2;//声明生存间隔为2 SURVIVE_INTERVAL
	...
	this.update = function(context, array, dt) {
		...
		//产生敌人开始
			...
			if (this.level >= 1 && this.muEyeNum == 0) {
				...
				if (this.muTimer >= this.muInterval) {
					...
					new MutativeEye(100, muPath, 15, 3).addTo(array);
					this.muTimer = 0;
					this.muEyeNum++;
				}
			}
			...
			if (this.bosslevel >= 1 && this.bossNum == 1) {
				...
				if (this.bosstime >= this.bossInterval) {
					...
					new BossPlane(65, bossmuPath,60, 3).addTo(array); //BOSS出现速度、路径、血量、子弹生成速度
					this.bosstime =0;
					this.bossNum--;
			...
			/*else{
				...
				var imges = new Image();
				imges.src = "img/Beat the game.jpg";
		...
		//}
			...
			if (this.bosslevel < 2) {
				if (this.score >= this.bosslevel * 200) {
					this.bosslevel++;
				}
			}
	...
	context.fillText("Score: " + this.score, 5, context.canvas.height - 30);
	context.fillText("Life: " + this.life, context.canvas.width - 80, context.canvas.height - 30);
		context.restore();

		document.getElementsByTagName("p")[1].innerHTML = "分数: " + this.score;
		document.getElementsByTagName("p")[2].innerHTML = "生命: " + this.life+"/15";
